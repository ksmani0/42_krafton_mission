...
int main(void)
{
    ...
    while (true)
    {
        ...
        system("cls");        
        thread statusThread(PrintState);
        thread printEnemyThread(MoveNPrintEnemy);
        thread printPlayerThread(PrintPlayer);
        thread movePlayerThread(MovePlayer);
        thread enemyShootThread(PrintEnemyShoot);
        thread playerShootThread(PrintPlayerShoot);

        //PlayerProcess();

        statusThread.join();
        printEnemyThread.join();
        printPlayerThread.join();
        movePlayerThread.join();
        enemyShootThread.join();
        playerShootThread.join();
